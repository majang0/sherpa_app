...
class UserActivityAnalyzer {
  ...
  ExercisePattern _analyzeExercisePattern(GlobalUser user, DateTime start) {
    ...
    // 상위 3개 운동 종류
    final mainTypes = typeFrequency.entries
        .toList()
    ...
    final preferredTimes = timeFrequency.entries
        .toList()
        ..sort((a, b) => b.value.compareTo(a.value));
    ...
    final totalIntensity = recentExercises.fold<double>(
      0.0, (sum, e) => sum + _getIntensityValue(e.intensity),
    );
    
    // 장소 추론 (운동 종류에서 추론)
    final locations = _inferExerciseLocations(mainTypes.take(3).map((e) => e.key).toList());
    ...
    return ExercisePattern(
      ...
      averageDuration: recentExercises.isEmpty ? 0 : totalDuration ~/ recentExercises.length,
      averageIntensity: recentExercises.isEmpty ? 0 : totalIntensity / recentExercises.length,
      locations: locations,
    );
  }
  ...
  ReadingPattern _analyzeReadingPattern(GlobalUser user, DateTime start) {
    ...
    final mainCategories = categoryFrequency.entries
        .toList()
        ..sort((a, b) => b.value.compareTo(a.value));
    ...
    for (var reading in recentReadings) {
      ...
      final period = hour >= 20 ? '밤' : hour >= 18 ? '저녁' : '오후';
      timeFrequency[period] = (timeFrequency[period] ?? 0) + 1;
    }
    ...
    // 최근 책들
    final recentBooks = recentReadings
        .take(5)
        .map((r) => r.bookTitle)
        .toList();
  ...
  MeetingPattern _analyzeMeetingPattern(GlobalUser user, DateTime start) {
    ...
    final totalSatisfaction = recentMeetings.fold(
      0.0, (sum, m) => sum + m.satisfaction,
    );
    final averageSatisfaction = totalSatisfaction / recentMeetings.length;
    
    // 최근 모임들
    final recentMeetingTitles = recentMeetings
        .take(5)
        .map((m) => m.meetingName)
        .toList();
  ...
  MoviePattern _analyzeMoviePattern(GlobalUser user, DateTime start) {
    ...
    for (var movie in recentMovies) {
      genreFrequency[movie.genre] = 
          (genreFrequency[movie.genre] ?? 0) + 1;
    }
    ...
    // 최근 영화들
    final recentMovieTitles = recentMovies
        .take(5)
        .map((m) => m.movieTitle)
        .toList();
  ...
  /// 시간대별 활동 패턴 추출
  Map<String, List<String>> _extractTimePatterns(GlobalUser user, DateTime start) {
  ...
  List<String> _extractPreferredLocations(GlobalUser user, DateTime start) {
    ...
    for (var exercise in exercises) {
      final inferredLocations = _inferLocationFromExercise(exercise.exerciseType);
    ...
    final sortedLocations = locationFrequency.entries
        .toList()
        ..sort((a, b) => b.value.compareTo(a.value));
  ...
  double calculateCategoryMatchScore(
    ...
    if (meetingCategory == MeetingCategory.outdoor) {
      final outdoorTypes = pattern.exercisePattern.mainTypes
          .where((t) => t.contains('등산') || t.contains('러닝') || 
                       t.contains('자전거'))
          .length;
      if (outdoorTypes > 0) {
        score += 0.3;
    ...
    // 기존 모임 참여 이력 반영
    if (pattern.meetingPattern.preferredCategories.contains(
        meetingCategory.displayName)) {
      score += 0.2;
    }
