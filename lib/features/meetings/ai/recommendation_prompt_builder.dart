...
''';
  ...
  String buildPrompt({
    ...
    if (userPattern.exercisePattern.frequency > 0) {
      ...
      buffer.writeln('- 주요 종류: ${userPattern.exercisePattern.mainTypes.join(", ")}');
      buffer.writeln('- 선호 시간대: ${userPattern.exercisePattern.preferredTimes.join(", ")}');
      buffer.writeln('- 평균 강도: ${userPattern.exercisePattern.averageIntensity.toStringAsFixed(1)}/5');
      if (userPattern.exercisePattern.locations.isNotEmpty) {
        buffer.writeln('- 추정 장소: ${userPattern.exercisePattern.locations.join(", ")}');
      }
      buffer.writeln();
    }
    ...
    if (userPattern.readingPattern.booksPerWeek > 0) {
      ...
      buffer.writeln('- 주요 카테고리: ${userPattern.readingPattern.mainCategories.join(", ")}');
      if (userPattern.readingPattern.preferredTimes.isNotEmpty) {
        buffer.writeln('- 선호 시간대: ${userPattern.readingPattern.preferredTimes.join(", ")}');
      }
      buffer.writeln('- 평균 평점: ${userPattern.readingPattern.averageRating.toStringAsFixed(1)}/5');
      if (userPattern.readingPattern.recentBooks.isNotEmpty) {
        buffer.writeln('- 최근 책: ${userPattern.readingPattern.recentBooks.take(3).join(", ")}');
      }
      buffer.writeln();
    }
    ...
    if (userPattern.meetingPattern.averagePerWeek > 0) {
      ...
      buffer.writeln('- 선호 카테고리: ${userPattern.meetingPattern.preferredCategories.join(", ")}');
      buffer.writeln('- 평균 만족도: ${userPattern.meetingPattern.averageSatisfaction.toStringAsFixed(1)}/5');
      if (userPattern.meetingPattern.preferredTimes.isNotEmpty) {
        buffer.writeln('- 선호 시간대: ${userPattern.meetingPattern.preferredTimes.join(", ")}');
      }
      buffer.writeln();
    }
    ...
    if (userPattern.moviePattern.moviesPerWeek > 0) {
      ...
      buffer.writeln('- 선호 장르: ${userPattern.moviePattern.preferredGenres.join(", ")}');
      buffer.writeln('- 평균 평점: ${userPattern.moviePattern.averageRating.toStringAsFixed(1)}/5');
      buffer.writeln();
    }
  ...
  String _formatMeetingForPrompt(AvailableMeeting meeting) {
    ...
    buffer.write('참가자: ${meeting.currentParticipants}/${meeting.maxParticipants} | ');
    ...
    if (meeting.description.isNotEmpty) {
      buffer.write(' | ${meeting.description.substring(0, 
          meeting.description.length > 50 ? 50 : meeting.description.length)}...');
    }
  ...
  List<Map<String, dynamic>> _parseTextResponse(String response) {
    ...
    for (var line in lines) {
      ...
      } else if (currentId != null && line.isNotEmpty && !line.startsWith('-')) {
        currentReason += ' $line';
      }
      ...
      return [
        {
          'meetingId': 'fallback',
          'matchScore': 0.5,
          'reason': '최근 2주간의 활동 데이터를 분석한 결과, 새로운 경험을 시도해보시면 좋을 것 같아요. 평소와 다른 시간대나 활동을 추천드립니다.',
          'keyPoints': [
            '새로운 경험 추천',
            '활동 다양성 확대',
            '성장 기회'
  ...
  String buildSimplePrompt({
    ...
    buffer.writeln('사용자: ${userPattern.userName} (Lv.${userPattern.userLevel})');
    ...
    buffer.writeln('\n모임 목록:');
    for (var meeting in availableMeetings.take(20)) { // 최대 20개만
      buffer.writeln('${meeting.id}|${meeting.category.displayName}|${meeting.title}|${meeting.location}');
    }
