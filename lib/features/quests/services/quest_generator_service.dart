...
class QuestGeneratorService {
  ...
  static List<QuestInstance> generatePremiumQuests() {
    ...
    while (selectedTemplates.length < 3 && attempts < 10) {
      ...
      // 해당 희귀도의 템플릿 중에서 선택
      final rarityTemplates =
          allPremiumTemplates.where((t) => t.rarity == targetRarity).toList();

      if (rarityTemplates.isNotEmpty) {
        final selected =
            rarityTemplates[_random.nextInt(rarityTemplates.length)];
    ...
    selectedQuests
        .addAll(selectedTemplates.map((t) => QuestInstance.fromTemplate(t)));

    return selectedQuests;
  }

  /// 템플릿 리스트에서 랜덤하게 count개 선택
  static List<QuestTemplate> _selectRandomTemplates(
      List<QuestTemplate> templates, int count) {
  ...
  /// 특정 유형의 퀘스트 모두 완료 확인
  static bool areAllQuestsCompleted(
      List<QuestInstance> quests, QuestTypeV2 type) {
  ...
  static bool areAllDailyQuestsCompleted(List<QuestInstance> quests) {
    final dailyQuests =
        quests.where((q) => q.type == QuestTypeV2.daily).toList();
  ...
  /// 카테고리별 퀘스트 분포 확인
  static Map<QuestCategoryV2, int> getCategoryDistribution(
  ...
  static QuestGenerationStats getGenerationStats(List<QuestInstance> quests) {
    ...
    final weeklyQuests =
        quests.where((q) => q.type == QuestTypeV2.weekly).length;
    final premiumQuests =
        quests.where((q) => q.type == QuestTypeV2.premium).length;
  ...
  static bool validateQuestGeneration(List<QuestInstance> quests) {
    ...
    final categoryValid =
        stats.categoryDistribution.values.every((count) => count > 0);
