...
class QuestConditionFormatter {
  ...
  static bool _needsFormatting(String description) {
    ...
    // 주간/이번 주 패턴 체크
    bool hasWeeklyPattern =
        (description.contains('이번 주') || description.contains('주간')) &&
    ...
    bool hasPremiumTechnicalTerms =
        lowerDesc.contains('consecutiveclimbingsuccess') ||
    ...
    return hasWeeklyPattern ||
        hasConditionPattern ||
        hasBasicTechnicalTerms ||
        hasPremiumTechnicalTerms;
  }
  ...
  static String _formatTechnicalTerms(String text) {
    ...
    final techTermReplacements = [
      ...
      {
        'pattern': RegExp(r'\bmeetingLogs?\b', caseSensitive: false),
        'replacement': '모임 참여'
      },
      {
        'pattern': RegExp(r'\bmovieLogs?\b', caseSensitive: false),
        'replacement': '영화 감상'
      },
      {
        'pattern': RegExp(r'\bmeetingReviews?\b', caseSensitive: false),
        'replacement': '모임 후기'
      },
      {
        'pattern': RegExp(r'\bmovieReviews?\b', caseSensitive: false),
        'replacement': '영화 리뷰'
      },
      {
        'pattern': RegExp(r'\breadingLogs?\b', caseSensitive: false),
        'replacement': '독서'
      },
      {
        'pattern': RegExp(r'\bexerciseLogs?\b', caseSensitive: false),
        'replacement': '운동'
      },
      {
        'pattern': RegExp(r'\breadingPages\b', caseSensitive: false),
        'replacement': '독서 페이지'
      },
      {
        'pattern': RegExp(r'\bdifferentMountains\b', caseSensitive: false),
        'replacement': '다른 산'
      },
      {
        'pattern':
            RegExp(r'\bdifferentMeetingCategories?\b', caseSensitive: false),
        'replacement': '다른 모임'
      },
      {
        'pattern': RegExp(r'\bexerciseMinutes\b', caseSensitive: false),
        'replacement': '운동 시간'
      },
      {
        'pattern': RegExp(r'\bfocusMinutes\b', caseSensitive: false),
        'replacement': '집중 시간'
      },
      {
        'pattern': RegExp(r'\bclimbingCompletions\b', caseSensitive: false),
        'replacement': '등반 완료'
      },
      {
        'pattern': RegExp(r'\bexerciseRecords\b', caseSensitive: false),
        'replacement': '운동 일수'
      },
      {
        'pattern': RegExp(r'\breadingRecords\b', caseSensitive: false),
        'replacement': '독서 일수'
      },
      {
        'pattern': RegExp(r'\bdiaryRecords\b', caseSensitive: false),
        'replacement': '일기 일수'
      },
      {
        'pattern': RegExp(r'\bappLaunches\b', caseSensitive: false),
        'replacement': '앱 사용 일수'
      },
      {
        'pattern': RegExp(r'\bpointsEarned\b', caseSensitive: false),
        'replacement': '포인트 획득'
      },
      {
        'pattern': RegExp(r'\bbadgeEquipped\b', caseSensitive: false),
        'replacement': '뱃지 장착'
      },
      {
        'pattern': RegExp(r'\bculturalRecords\b', caseSensitive: false),
        'replacement': '문화 활동'
      },

      // 프리미엄 퀘스트용 복합 조건 용어들
      {
        'pattern': RegExp(r'\b연속등반성공\b', caseSensitive: false),
        'replacement': '연속 등반 성공'
      },
      {
        'pattern': RegExp(r'\b연속일일퀘스트완료\b', caseSensitive: false),
        'replacement': '연속 일일 퀘스트 완료'
      },
      {
        'pattern': RegExp(r'\b모임주최성공\b', caseSensitive: false),
        'replacement': '참여자가 있는 모임 주최'
      },
      {
        'pattern': RegExp(r'\bdifferentExerciseTypes\b', caseSensitive: false),
        'replacement': '다른 운동 종류'
      },
      {
        'pattern': RegExp(r'\bdifferentMeetings\b', caseSensitive: false),
        'replacement': '다른 모임'
      },
      {
        'pattern': RegExp(r'\bperfectDays\b', caseSensitive: false),
        'replacement': '완벽한 하루'
      },
      {
        'pattern': RegExp(r'\ballActivitiesDays\b', caseSensitive: false),
        'replacement': '모든 활동 완료일'
      },
      {
        'pattern': RegExp(r'\bchallengeRecords\b', caseSensitive: false),
        'replacement': '챌린지 참여'
      },
      {
        'pattern': RegExp(r'\b30일챌린지첫주\b', caseSensitive: false),
        'replacement': '30일 챌린지 첫 주 완료'
      },
      {
        'pattern': RegExp(r'\b모든카테고리퀘스트완료\b', caseSensitive: false),
        'replacement': '모든 카테고리 퀘스트 완료'
      },
      {
        'pattern': RegExp(r'\b모든주간퀘스트완료\b', caseSensitive: false),
        'replacement': '모든 주간 퀘스트 완료'
      },

      // 추가 패턴들
      {
        'pattern': RegExp(r'\bweeklyReadingPages\b', caseSensitive: false),
        'replacement': '주간 독서 페이지'
      },
      {
        'pattern': RegExp(r'\bweeklyMovieLogs\b', caseSensitive: false),
        'replacement': '주간 영화 감상'
      },
      {
        'pattern': RegExp(r'\bweeklyMeetingReviews\b', caseSensitive: false),
        'replacement': '주간 모임 후기'
      },
      {
        'pattern': RegExp(r'\bweeklyDifferentMeetingCategories\b',
            caseSensitive: false),
        'replacement': '주간 다른 모임 카테고리'
      },
      {
        'pattern':
            RegExp(r'\bweeklyDifferentMountains\b', caseSensitive: false),
        'replacement': '주간 다른 산'
      },
    ];

    for (final replacement in techTermReplacements) {
      formatted = formatted.replaceAll(replacement['pattern'] as RegExp,
    ...
    switch (path) {
      ...
      default:
        ...
        if (condition.description.contains('조건 달성:')) {
          ...
          naturalDesc =
              naturalDesc.replaceAll('dailyRecords.todaySteps', '오늘 걸음수');
          naturalDesc = naturalDesc.replaceAll(
              'dailyRecords.todayExerciseMinutes', '오늘 운동 시간');
          naturalDesc = naturalDesc.replaceAll(
              'allDailyActivitiesCompleted', '모든 일일 활동 완료');
          naturalDesc =
              naturalDesc.replaceAll('dailyPointsEarned', '오늘 포인트 획득');
