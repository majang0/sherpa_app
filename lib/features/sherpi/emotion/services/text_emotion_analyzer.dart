...
class TextEmotionAnalyzer {
  ...
  static EmotionSnapshot analyzeText(
    ...
    final dominantEmotion =
        _selectDominantEmotion(emotionScores, patternAnalysis);
  ...
  static Map<String, dynamic> _analyzeTextPatterns(String text) {
    return {
      ...
      // 이모지/이모티콘 분석 (간단한 패턴)
      'positive_emoticons':
          RegExp(r'[ㅎㅋㅠㅜ]{2,}|하하|헤헤|히히').allMatches(text).length,
      'negative_emoticons': RegExp(r'ㅠㅠ|ㅜㅜ|엉엉|흑흑').allMatches(text).length,

      // 강조 표현
      'emphasis':
          RegExp(r'진짜|정말|완전|엄청|너무|아주|매우|굉장히|정말로').allMatches(text).length,
  ...
  static EmotionIntensity _calculateIntensity(
    ...
    // 강조 표현 분석
    final emphasisCount =
        RegExp(r'진짜|정말|완전|엄청|너무|아주|매우|굉장히|대박|미친').allMatches(text).length;
  ...
  static EmotionConfidence _calculateConfidence(
    ...
    final patternCount =
        patterns.values.where((value) => value is int && value > 0).length;
  ...
  static Map<String, dynamic> getAnalysisSummary(String text) {
    ...
    return {
      ...
      'multiple_emotions': multipleEmotions
          .map((e) => {
                'type': e.type.id,
