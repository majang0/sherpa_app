...
class AiInsightGenerator {
  ...
  Future<List<Recommendation>> generateAIRecommendations(
    ...
    try {
      ...
      // 기존 추천사항과 결합
      final combinedRecommendations = [...analysisResult.recommendations, ...aiRecommendations];
...
''';
  ...
  /// 성과 요약 생성
  String _buildPerformanceSummary(GlobalUser user, AnalysisResult analysisResult) {
    final metrics = analysisResult.performanceMetrics;
...
''';
  ...
  String _processGrowthPlan(String aiResponse) {
    ...
    // 불필요한 접두사/접미사 제거
    processed = processed.replaceAll(RegExp(r'^(안녕하세요|안녕|네,?\s*)', caseSensitive: false), '');
    processed = processed.replaceAll(RegExp(r'(감사합니다|고맙습니다)\.?\s*$', caseSensitive: false), '');
