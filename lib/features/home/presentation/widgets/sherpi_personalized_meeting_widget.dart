...
class _SherpiPersonalizedMeetingWidgetState
  ...
  Map<String, dynamic> _analyzeUserActivity() {
    ...
    List<MapEntry<MeetingCategory, double>> sortedCategories = categoryScores.entries
        .where((e) => e.value > 0)  // 점수가 0보다 큰 카테고리만
        .toList()
      ..sort((a, b) => b.value.compareTo(a.value));  // 점수 높은 순으로 정렬
    
    // 상위 3개 카테고리 추출 (없으면 있는 만큼만)
    List<MeetingCategory> topCategories = sortedCategories
        .take(3)
        .map((e) => e.key)
        .toList();
    ...
    return {
      'primaryActivity': _getPrimaryActivity(recentReadings, recentExercises, recentMeetings),
      'insight': insight,
      ...
      'totalActivities': recentReadings.length + recentExercises.length + recentMeetings.length,
    };
  }
  ...
  List<MeetingCategory> _mapExerciseToMeetingCategories(String exerciseType) {
    ...
    if (lowerType.contains('러닝') || lowerType.contains('달리기') || 
        lowerType.contains('조깅') || lowerType.contains('마라톤')) {
  ...
  String _generatePersonalizedInsight(
    ...
    final totalActivities = readings.length + exercises.length + meetings.length;
    ...
    } else if (readings.length >= exercises.length && readings.length >= meetings.length) {
      // 독서가 주요 활동인 경우
  ...
  String _getPrimaryActivity(
    ...
    if (readings.length >= exercises.length && readings.length >= meetings.length) {
      return '독서';
    } else if (exercises.length >= readings.length && exercises.length >= meetings.length) {
      return '운동';
  ...
  AvailableMeeting? _selectRecommendedMeeting(Map<String, dynamic> analysis) {
    ...
    // 추천된 카테고리와 점수 가져오기
    final recommendedCategory = analysis['recommendedCategory'] as MeetingCategory;
    final categoryScores = analysis['categoryScores'] as Map<MeetingCategory, double>;
    ...
    if (recommendedCategoryMeetings.isNotEmpty) {
      // 추천 카테고리 내에서 점수 계산
      final scoredRecommendedMeetings = recommendedCategoryMeetings.map((meeting) {
    ...
    final validMeetings = scoredMeetings
        .where((e) => e.value > 0)
        .take(5)
        .toList();
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildPremiumHeader(context, analysis['emotion'] as SherpiEmotion),
