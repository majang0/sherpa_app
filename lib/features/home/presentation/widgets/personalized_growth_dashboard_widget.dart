...
/// 상수 정의
class _Constants {
  static const List<String> goalIds = [
    'steps',
    'focus',
    'reading',
    'exercise',
...
class _PersonalizedGrowthDashboardWidgetState
  ...
  Widget _buildIntegratedHeaderSection(
      GlobalUser user, List<DailyGoal> dailyGoals) {
    ...
    return Container(
      padding: const EdgeInsets.fromLTRB(
          _Constants.headerPadding, 24, _Constants.headerPadding, 24),
      decoration: BoxDecoration(
      ...
      child: Column(
        children: [
          ...
          // 🎯 중앙 대형 진행률 표시
          _buildHeroCircularProgress(
              progress, isAllCompleted, completedCount, totalCount),
        ],
      ),
    );
  }
  ...
  // 🎯 히어로 스타일 대형 원형 진행률
  Widget _buildHeroCircularProgress(double progress, bool isAllCompleted,
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            // 🏆 완성시 황금색, 미완성시 화이트 배경
            color: isAllCompleted ? ModernColors.streakGold : Colors.white,
  ...
  Widget _buildGoalsGrid(List<DailyGoal> dailyGoals) {
    final records =
        ref.watch(globalUserProvider.select((user) => user.dailyRecords));
  ...
  Widget _buildRewardDisplayCard() {
    final isAllGoalsRewardClaimed = ref.watch(globalUserProvider
        .select((user) => user.dailyRecords.isAllGoalsRewardClaimed));
    final canClaimReward =
        _checkAllGoalsCompleted() && !isAllGoalsRewardClaimed;

    Widget cardContent = Container(
      padding: const EdgeInsets.symmetric(
          horizontal: 16, vertical: 12), // 클릭 가능할 때 더 넓게
      decoration: BoxDecoration(
  ...
  Widget _buildCompactGoalCard(
      String goalId, DailyRecordData records, bool isCompleted) {
    final goalData = _getGoalData(goalId);

    // 🛡️ 보상받기 상태 확인 (프로바이더 최적화)
    final isAllGoalsRewardClaimed = ref.read(globalUserProvider
        .select((user) => user.dailyRecords.isAllGoalsRewardClaimed));
    final canClaimReward =
        _checkAllGoalsCompleted() && !isAllGoalsRewardClaimed;
  ...
  List<bool> _calculateWeeklyCompletionStatus(GlobalUser user) {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      weeklyStatus.add(
          _checkAllGoalsRewardClaimedForDate(user.dailyRecords, checkDate));
    }
  ...
  bool _checkAllGoalsRewardClaimedForDate(
      DailyRecordData records, DateTime checkDate) {
    // ✅ 실제 보상 받은 날짜 리스트를 확인
