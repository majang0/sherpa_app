...
import '../../features/quests/models/quest_instance_model.dart';
...
class ActivityDataCollector {
  ...
  Map<String, dynamic> collectExerciseData({
    ...
    return {
      ...
      'todayStats': {
        ...
        'totalMinutes': todayExercises.fold<int>(0, (sum, log) => sum + log.durationMinutes) + durationMinutes,
        'exerciseTypes': [...exerciseTypeStats.keys, exerciseType].toSet().toList(),
        'averageIntensity': _calculateAverageIntensityScore(todayExercises, intensity),
      ...
      'motivation': {
        ...
        'comparisonToAverage': _compareToUserAverage(user, 'exercise', durationMinutes),
      },
  ...
  Map<String, dynamic> collectStudyData({
    ...
    return {
      ...
      'todayStats': {
        ...
        'totalPages': todayReading.fold<int>(0, (sum, log) => sum + log.pages) + pages,
  ...
  Map<String, dynamic> collectQuestData({
    ...
    return quests.when(
      data: (questList) {
        final completedQuests = questList.where((q) => q.status == QuestStatus.completed).toList();
        ...
        return {
          ...
          'rewards': {
            ...
            'totalPointsToday': _getTodayEarnedPoints(points.transactions) + rewardPoints,
      ...
      loading: () => _getDefaultQuestData(questName, questType, rewardPoints, difficulty),
      error: (_, __) => _getDefaultQuestData(questName, questType, rewardPoints, difficulty),
    );
  }
  ...
  Map<String, int> _calculateDetailedStreak(GlobalUser user, String activityType) {
    // 실제 구현에서는 연속 일수 계산
    return {
      'current': 3,
      'longest': 7,
  ...
  double _calculateAverageIntensityScore(List<ExerciseLog> exercises, String currentIntensity) {
    if (exercises.isEmpty) {
      return _intensityToScore(currentIntensity);
    }
    
    double totalScore = exercises.fold(0.0, (sum, log) => sum + _intensityToScore(log.intensity));
  ...
  String _compareToUserAverage(GlobalUser user, String activity, int value) => 'above_average';
  ...
  double _calculateReadingSpeed(List<ReadingLog> logs, int currentPages) => 30.0; // pages per hour
  ...
  double _calculateNewAverageRating(List<ReadingLog> logs, double? rating) {
    ...
    final existingRatings = logs.where((l) => l.rating != null).map((l) => l.rating!).toList();
  ...
  int _estimateReadingTime(int pages) => (pages * 2).round(); // 2 minutes per page
  ...
  double _calculateReadingDiversityScore(Map<String, dynamic> preferences) => 0.7;
  ...
  double _calculateEmotionalAwarenessScore(Map<String, dynamic> patterns) => 0.75;
  double _calculateExpressivenessScore(String content, Map<String, dynamic> data) => 
      content.length > 200 ? 0.8 : 0.5;
  double _analyzeReflectionDepth(String content) => content.length > 300 ? 0.9 : 0.6;
  ...
  double _calculateQuestCompletionRate(List<QuestInstance> quests) {
    ...
    final completed = quests.where((q) => q.status == QuestStatus.completed).length;
  ...
  int _estimateTotalConnections(List<MeetingLog> meetings, int currentParticipants) => 
