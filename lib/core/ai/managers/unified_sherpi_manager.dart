...
import 'package:sherpa_app/core/ai/strategies/emotion_selection_strategy.dart';
...
class UnifiedSherpiManager implements SherpiMessageManager {
  ...
  final EmotionSelectionStrategy _emotionStrategy = EmotionSelectionStrategy();
  ...
  Future<SherpiResponse> _getAIMessage(
    ...
    try {
      ...
      // 2. 🎯 메시지 타입 선택 (Strategy Pattern 사용)
      final selectedType = _emotionStrategy.selectMessageType(
      ...
      // 🔒 보안: PII 제거, 메타데이터만 로깅 (GDPR 준수)
      if (kDebugMode) {
        // 개발 환경: 메타데이터만 로깅 (PII 제외)
        final sanitizedContext = {
          'hasUserName': gameContext?['userName'] != null,
          'level': gameContext?['level'],
          'consecutiveDays': gameContext?['consecutiveDays'],
          'fieldCount': gameContext?.keys.length ?? 0,
          'messageType': selectedType.korean,
        };
        aiLogger.d('📊 AI 메시지 생성 메타데이터: $sanitizedContext');
      }
      ...
      // 7. 🎯 메시지 타입 히스토리 업데이트 (Strategy 사용)
      _emotionStrategy.recordSelection(selectedType);

      aiLogger.i('✅ AI 격려 메시지 생성 성공 (${duration.inMilliseconds}ms)');
      aiLogger.i('📝 최근 메시지 타입: ${_emotionStrategy.recentMessageTypes}');
...
''';
  ...
  // ═══════════════════════════════════════════════════════════════
  // 🎯 메시지 타입 선택 로직이 EmotionSelectionStrategy로 이동됨
  // (Strategy Pattern 적용으로 코드 중복 제거 및 유지보수성 향상)
  // ═══════════════════════════════════════════════════════════════
