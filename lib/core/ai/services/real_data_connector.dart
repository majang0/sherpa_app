...
class RealDataConnector {
  ...
  Map<String, dynamic> buildRealUserContext({
    ...
    final userContext = <String, dynamic>{
      ...
      // 오늘의 활동 기록
      'todayExerciseMinutes': dailyRecord.exerciseLogs
          .fold<int>(0, (sum, log) => sum + log.durationMinutes),
      'todayReadingPages':
          dailyRecord.readingLogs.fold<int>(0, (sum, log) => sum + log.pages),
      ...
      'longestStreak': user.dailyRecords
          .consecutiveDays, // longestStreak가 없으므로 consecutiveDays 사용
  ...
  Map<String, dynamic> _getExerciseContextData(
      GlobalUser user, Map<String, dynamic>? additionalData) {
    final todayExercises = user.dailyRecords.exerciseLogs;

    return {
      'todayExerciseCount': todayExercises.length,
      'totalExerciseMinutes':
          todayExercises.fold<int>(0, (sum, log) => sum + log.durationMinutes),
      'exerciseTypes':
          todayExercises.map((e) => e.exerciseType).toSet().toList(),
      'averageIntensity': todayExercises.isEmpty
          ? 0
  ...
  Map<String, dynamic> _getStudyContextData(
    ...
    return {
      ...
      'totalPagesRead':
          todayReading.fold<int>(0, (sum, log) => sum + log.pages),
      'bookTitles': todayReading.map((r) => r.bookTitle).toList(),
      'averageRating': todayReading.where((r) => r.rating != null).isEmpty
          ? 0
          : todayReading
  ...
  /// 🎯 퀘스트 컨텍스트 데이터
  Map<String, dynamic> _getQuestContextData(
      Map<String, dynamic>? additionalData) {
    final quests = _ref.read(questProviderV2);
  ...
  String _determineActivityPattern(
      List<int> exerciseTimes, List<int> studyTimes) {
  ...
  String _estimateEnergyLevel(int hour, DailyRecordData todayRecord) {
    final activityCount =
        todayRecord.exerciseLogs.length + todayRecord.readingLogs.length;
