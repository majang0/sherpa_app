...
class ExerciseCalculator {
  ...
  static Map<String, dynamic> calculateHeartRateEffect({
    ...
    final targetHR =
        ((maxHR - restingHR) * targetPercentage + restingHR).round();
  ...
  static Map<String, dynamic> getBloodPressureEffect({
    ...
    final actualReduction =
        (systolicReduction * timeMultiplier).toStringAsFixed(1);
    final diastolicReduction =
        (systolicReduction * 0.7 * timeMultiplier).toStringAsFixed(1);
    ...
    return {
      ...
      'longTermBenefit':
          durationMinutes >= 30 ? '심혈관 질환 위험 20% 감소' : '심혈관 건강 개선',
    };
  }
  ...
  static Map<String, dynamic> getEndorphinEffect({
    ...
    if (durationMinutes >= onsetTime) {
      endorphinLevel =
          math.min(((durationMinutes - onsetTime) / 10 * 20).round(), 100);
    }
  ...
  static Map<String, dynamic> getBrainEffect({
    ...
    final isAerobic =
        ['러닝', '달리기', '조깅', '수영', '자전거', '에어로빅'].contains(exerciseType);
  ...
  static Map<String, dynamic> getSleepEffect({
    ...
    return {
      ...
      'effect': effect,
      'deepSleep':
          sleepQuality >= 20 ? '${(sleepQuality / 10).round()}시간 증가' : '변화 없음',
