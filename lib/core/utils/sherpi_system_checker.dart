...
class SherpiSystemChecker {
  /// 🧪 전체 시스템 상태 확인
  static Future<Map<String, dynamic>> checkSystemIntegration(
      WidgetRef ref) async {
  ...
  /// 4️⃣ 메시지 생성 테스트
  static Future<Map<String, dynamic>> _testMessageGeneration(
      WidgetRef ref) async {
    try {
      ...
      testResults['welcome_message_generated'] =
      ...
      testResults['metadata_available'] =
          currentState.metadata?.isNotEmpty ?? false;
  ...
  static String summarizeSystemStatus(Map<String, dynamic> results) {
    ...
    final providerResults =
        results['provider_initialized'] as Map<String, dynamic>?;
    ...
    if (aiResults != null) {
      ...
      buffer.writeln(
          '  ⭐ AI 레벨 수: ${aiResults['ai_levels_count']}단계 (premium/smart/basic)');
    ...
    if (memoryResults != null) {
      ...
      buffer
          .writeln('  📨 메시지 캐시 제한: ${memoryResults['message_cache_limit']}개');
      buffer.writeln(
          '  📊 품질 메트릭 제한: ${memoryResults['quality_metrics_limit']}개');
      buffer
          .writeln('  🎭 템플릿 캐시 제한: ${memoryResults['template_cache_limit']}개');
    }

    // 성능 벤치마크 결과
    final perfResults =
        results['performance_benchmark'] as Map<String, dynamic>?;
    if (perfResults != null && perfResults['benchmark_executed'] == true) {
      buffer.writeln('');
      buffer.writeln('🚀 Phase 1 성능 벤치마크:');
      buffer.writeln(
          '  🏆 성능 점수: ${(perfResults['performance_score'] as double).toStringAsFixed(1)}/100');
      buffer.writeln('  📊 등급: ${perfResults['score_grade']}');
      buffer.writeln(
          '  🎯 목표 달성: ${perfResults['targets_met']}/${perfResults['total_targets']}개 (${perfResults['targets_met_percentage']}%)');
  ...
  static Future<Map<String, dynamic>> _runPerformanceBenchmark(
      WidgetRef ref) async {
    try {
      // 가볍고 빠른 벤치마크 (5회 반복)
      ...
      final performanceScore =
          benchmarkResults['performance_score'] as double? ?? 0.0;
      final meetsTargets =
          benchmarkResults['meets_targets'] as Map<String, bool>? ?? {};
      ...
      return {
        ...
        'targets_met_percentage': meetsTargets.isNotEmpty
            ? (meetsTargets.values.where((met) => met).length /
                    meetsTargets.length *
                    100)
                .round()
            : 0,
  ...
  static Future<String> generatePhase1BenchmarkReport(WidgetRef ref) async {
    try {
      ...
      return Phase1PerformanceBenchmark.generateBenchmarkReport(
          benchmarkResults);
    } catch (e) {
